...
def generate_response(messages, model="gpt-4o-mini", max_tokens=300):
...
def query_requires_history(current_query, previous_query, previous_answer):
    print(f"\nDEBUG: Analyzing query for history requirement: '{current_query}'")
    print(f"DEBUG: Previous query: '{previous_query}'")
    print(f"DEBUG: Previous answer: '{previous_answer}'")
    
    system_content = """ 
    Your task is to determine if the current query requires context from the previous conversation to be answered accurately and completely.
    Consider the relationship between the current query, the previous query, and its answer.
    Look for references to previous information, follow-up questions, or implicit context that relies on the previous exchange.
    """
    ...
    Does the current query require information from the previous conversation to be answered accurately? 
    Respond with only True or False, followed by a brief explanation."""
    ...
    response = generate_response(messages)
    print(f"DEBUG: Model's response for history requirement: {response}")
    
    # Extract the True/False decision and explanation
    decision, explanation = response.strip().split('\n', 1) if '\n' in response else (response.strip(), "")
    requires_history = decision.lower() == 'true'
    
    print(f"DEBUG: Query {'requires' if requires_history else 'does not require'} historical context.")
    print(f"DEBUG: Explanation: {explanation.strip()}")

    return requires_history

def generate_context_from_history(query):
    if not query_requires_history(query):
        return ""
...
def parse_and_expand_query(query):
    print(f"DEBUG: Entering parse_and_expand_query with query: {query}")
    ...
    print("DEBUG: Sending request to generate_response")
    response = generate_response(messages)
    print(f"DEBUG: Received response from generate_response: {response}")
    ...
    try:
        ...
        # Check if query_intent is "stats" but required_tables is empty
        if parsed_data["query_intent"] == "stats" and not parsed_data["required_tables"]:
            print("DEBUG: Query intent is stats but no tables selected. Sending for table selection.")
            # Send the JSON back to the model for table selection
            table_selection_prompt = f"""The following query requires statistics, but no tables were selected. Please analyze the query again and select the most appropriate table(s) based on the statistics requested:
            ...
            table_selection_response = generate_response(table_selection_messages)
            print(f"DEBUG: Received table selection response: {table_selection_response}")
            cleaned_table_selection_response = clean_response(table_selection_response)
            parsed_data = json.loads(cleaned_table_selection_response)

        # Add required columns to the parsed data
        parsed_data["required_columns"] = {}
        for table in parsed_data["required_tables"]:
            if table in simplified_hockey_stats_schema:
                parsed_data["required_columns"][table] = list(simplified_hockey_stats_schema[table].keys())

        print(f"DEBUG: Final parsed_data: {json.dumps(parsed_data, indent=2)}")
        return parsed_data
    except json.JSONDecodeError as e:
        print(f"DEBUG: JSON decode error: {str(e)}")
        print(f"DEBUG: Raw response causing the error: {response}")
        return None


def generate_sql_query(analyzed_data):
    system_content = """You are a SQL expert specialized in querying hockey statistics databases. 
    Generate a PostgreSQL query based on the provided information. Follow these rules:
    ...
    # Additional safety check to ensure the assist column is correctly referenced
    sql_query = sql_query.replace("i_f_assists", "(i_f_primaryassists + i_f_secondaryassists)")
    ...
    if 'player_stats_regular_season' in analyzed_data['required_tables'] or 'player_stats_playoffs' in analyzed_data['required_tables']:
        ...
        # Ensure the situation is included in the WHERE clause
        if "WHERE" in sql_query and "situation" not in sql_query:
            sql_query = sql_query.replace("WHERE", f"WHERE situation = '{analyzed_data['situation']}' AND")
        elif "WHERE" not in sql_query:
            sql_query += f" WHERE situation = '{analyzed_data['situation']}'"
        
        # Add a check to set situation to 'all' if not defined
        if "situation = " not in sql_query:
            sql_query += " WHERE situation = 'all'"
    
    return sql_query

def test_sql_query(query, analyzed_data):
    db_params = {
        "dbname": "hockey_stats",
        "user": "sauhumatti",
        "host": "localhost"
        ...
        with psycopg2.connect(**db_params) as conn:
            ...
            with conn.cursor() as cursor:
                try:
                    ...
                    corrected_query = correct_query(query, error_message, analyzed_data)
                    if corrected_query != query:
                        print("Attempting with corrected query:")
...
def correct_query(query, error_message, analyzed_data):

    # If the error is not handled by the above, use the AI model to correct the query
    system_content = """You are a SQL expert specialized in correcting and optimizing queries for hockey statistics databases. 
    Your task is to analyze the given SQL query, the error message, and the original query requirements, then generate a corrected SQL query.
    Follow these guidelines:
...
def generate_natural_language_answer_non_hockey(original_query, expanded_query):
    system_content = """Answer to the user as normally but
    if the query is not about hockey, suggest to the user to ask something hockey statistics related. 
    The database contains player and team statistics from 2008-2024.
    You can suggest questions to ask if the user is lost.
...
def generate_natural_language_answer_hockey_without_data(original_query, parsed_result):
    system_content = """You are an AI assistant and you know everything about the NHL. 
    Provide the user with a general answer based on common hockey knowledge.
    Use the information provided in the parsed query to give a more accurate and relevant response.
    If specific players or teams are mentioned, include information about them in your answer."""
...
def generate_natural_language_answer_with_data(original_query, expanded_query, sql_query, query_result):
    # Generate table if query_result is available
    table = ""
    if query_result:
        try:
            # Assuming query_result is a list of dictionaries
            headers = query_result[0].keys()
            rows = [list(row.values()) for row in query_result]
...
def generate_error_response(query, expanded_query, error_message):
    system_content = """You are an AI assistant specialized in hockey statistics. 
    An error occurred while trying to fetch specific data for the user's query. 
    Your task is to:
    1. Inform the user about the error in fetching the data.
...
def handle_non_hockey_query(query, result):
    print("\nQuery is not related to hockey statistics. Skipping SQL query generation and testing.")
    nl_answer = generate_natural_language_answer_non_hockey(
        query, 
        result['expanded_query']
    )
...
def handle_hockey_query_without_data(query, result):
    print("\nQuery is hockey-related but doesn't require specific table data. Generating general answer.")
    nl_answer = generate_natural_language_answer_hockey_without_data(
        query, 
        result  # Pass the full parsed result instead of just the expanded query
    )
...
def handle_hockey_query_with_data(query, result):
    sql_query = generate_sql_query(result)
    print("\nGenerated SQL Query:")
    print(sql_query)
...
def handle_failed_query():
    print("Failed to parse and expand query.")
    response = {
        "natural_language_answer": "Failed to parse and expand query.",
        "hockey_related": None
    }
...
def process_query(query):
    ...
    # Generate context from history if needed
    context = generate_context_from_history(query)
    
    # Combine context and query for parsing
    full_query = f"{context}\n\nCurrent query: {query}" if context else query
...
# Add a debug flag
DEBUG = True  # Set to False to disable console printing
...
def main():
    ...
    while True:
        ...
        result = parse_and_expand_query(user_query)
        print(json.dumps(result, indent=2))
